...
const Deposit = () => {
  ...
  const amounts = [1000, 1500, 2000, 3000, 5000];

  // Read query parameters to auto-fill amount
  useEffect(() => {
  ...
  const handleDeposit = async () => {
    try {
      ...
      const res = await userService.getPaymentUrl({amount:amount});
        ...
        {amounts.map((amt) => (
          <button
            ...
            style={{
              ...
              cursor: "pointer",
              borderRadius: "5px",
              background: amount === amt ? "#28a745" : "#fff",
